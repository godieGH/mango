...
kazi resolve_all(specs, target_root) {
  ...
  data graph = {
    root: [],           // Root-level packages ["math@1.2.3", ...]
    packages: {},       // All resolved packages: "math@1.2.3" -> metadata
    edges: {},         // Dependencies: "math@1.2.3" -> ["fmt@2.0.0", ...]
  ...
  wakati !queue.empty() {
    ...
    data { name, range, parent, depth } = current
...
kazi download_packages(lockdata, target_root) {
  ...
  kama !fs.exists(cache_dir) {
    fs.mkdir(cache_dir, { recursive: kweli })
  }
  ...
  kwa kila (pkg_key, info) ktk lockdata.packages {
    ...
    kama fs.exists(cache_path) {
      // Verify integrity
      data cached_data = fs.readFile(cache_path, { encoding: "binary" })
    ...
    jaribu {
      ...
      // Save to cache
      fs.writeFile(cache_path, response, { encoding: "binary" })
...
kazi extract_packages(lockdata) {
  ...
  kama !fs.exists(vendor_dir) {
    fs.mkdir(vendor_dir, { recursive: kweli })
  }
  ...
  kwa kila (pkg_key, info) ktk lockdata.packages {
    ...
    jaribu {
      ...
      fs.mkdir(pkg_vendor_path, { recursive: kweli })
      
      // Extract tarball
      data tarball = fs.readFile(cache_path, { encoding: "binary" })
      ...
      kwa kila (file, idx) ktk extracted_data {
        ...
        kama !fs.exists(dir) {
          fs.mkdir(dir, { recursive: kweli })
        }
        fs.writeFile(full_path, content, { encoding: "binary" })
      }
...
kazi link_dependencies(lockdata, target_root, is_global) {
  ...
  kwa kila (pkg_key, info) ktk lockdata.packages {
    ...
    kama !fs.exists(pkg_vendor_deps) {
      fs.mkdir(pkg_vendor_deps, { recursive: kweli })
    }
  ...
  kama is_global {
    ...
    kama !fs.exists(globals_dir) {
      fs.mkdir(globals_dir, { recursive: kweli })
    }
    ...
    kama !fs.exists(project_vendor) {
      fs.mkdir(project_vendor, { recursive: kweli })
    }
...
// ========================================
kazi async build_packages(lockdata, graph) {
  ...
  kwa kila pkg_key ktk graph.buildRequired {
    ...
    data req_check = await check_build_requirements(info.manifest)
...
kazi load_lockfile(target_root) {
  ...
  jaribu {
    data lock_content = fs.readFile(lock_path, { encoding: "utf8" })
...
kazi async install_package(cli) {
  ...
  jaribu {
    ...
    kama graph na graph.buildRequired {
      await build_packages(lockdata, graph)
    }
...
kazi load_global_manifest() {
  ...
  jaribu {
    data content = fs.readFile(manifest_path, { encoding: "utf8" })
...
kazi async check_build_requirements(manifest) {
  ...
  kama !manifest.build {
    rudisha { satisfied: kweli, missing: [] }
  }
...
kazi async build_native_addon(pkg_name, pkg_version, pkg_path, build_config) {
  ...
  kama !fs.exists(build_path) {
    fs.mkdir(build_path, { recursive: kweli })
  }
